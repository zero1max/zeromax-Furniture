...
def shop(request):
    ...
    paginator = Paginator(furnitures, 4)  # Har bir sahifada 10 ta mebel ko'rsatiladi
...
def search(request):
    ...
    results = Furniture.objects.filter(title__icontains=query, is_published=True) if query else []
    context = {
        'query': query,
        'results': results,
        'title':'Search'
    }
    return render(request, 'furniture/search.html', context)

class FurnitureView(ListAPIView):
    queryset = Furniture.objects.all()
    serializer_class = FurnitureSerializer

class FurnitureViewCRUD(RetrieveUpdateDestroyAPIView):
    queryset = Furniture.objects.all()
    serializer_class = FurnitureSerializer
